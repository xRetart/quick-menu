...
impl Query {
    ...
    pub fn render<B: Backend>(&self, frame: &mut Frame<B>, destination: Rect) {
        ...
        let widget = Paragraph::new(self.string.clone()).block(block).scroll((0, scroll));
    ...
    fn cursor(&self, width: u16) -> u16 {
        let length = u16::try_from(self.string.len()).unwrap();
        (length + 1).min(width - 3)
    }
    fn scroll(&self, width: u16) -> u16 {
        let length = i64::try_from(self.string.len()).unwrap();
